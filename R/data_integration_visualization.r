...
ggplot() + 
  ...
  ggtitle("Programas de Pós-GraduaÃ§Ã£o em Computação") +
...
ggplot() + 
  geom_polygon( data = shape_brazil, aes(x = long, y = lat, group = group), fill = "grey", alpha = 0.3) +
  geom_path( data = shape_brazil, aes( x = long, y = lat, group = group), color = "black", size = 0.1) +
  geom_point( data = all_data[all_data$nome=="Inteligência Artificial",], aes(x = long_university, y = lat_university, size = as.factor(Nota), color = as.factor(Nota), alpha = 0.7)) +
...
ggplot() + 
  ...
  coord_map() + 
  guides( colour = guide_legend(), alpha = FALSE) +
  ggtitle("Programas de Pós-Graduação em Teoria da ComputaÃ§Ã£o") +
  labs(x = "Longitude", y = "Latitude") +
  theme(
    legend.position = c(0.9, 0.15),
    text = element_text(color = "#22211d"),
...
ggplot() + 
  ...
  ggtitle("Programas de PÃ³s-Graduação com Mestrado e Doutorado") +
  labs(x = "Longitude", y = "Latitude") +
  theme(
    legend.position = c(0.9, 0.15),
    text = element_text(color = "#22211d"),
